...
% Unlike Campbell's original function, we'll assume that the user is only
% going to be plotting one set of data at each x location. We'll pick
% different colors for each x position so it's easier to tell each dataset
% apart.

% Use cbrewer to create a color map for each of the different columns.
num_cols = size(y,2);
[cols] = cbrewer('qual','Dark2',num_cols);
%cols = cols(randperm(num_cols),:); %optional - can shuffle colors so they
                                    %won't be sequential
...
%     end
    ...
    % Reorder so that dots go below mean/sem lines
    uistack(h(k).data, 'bottom');
...
function [errs1,errs2] = errorBars(thisInterval)
    l=mu(k)-thisInterval;
    u=mu(k)+thisInterval;
    errs1=line([X(k)-jitScale/2 X(k)+jitScale/2],...
           [l l]);
    errs2=line([X(k)-jitScale/2 X(k)+jitScale/2],...
           [u u]);
    errs3=line([X(k) X(k)],[l u]);
    set([errs1,errs2,errs3],'color',[0 0 0],'linewidth',1)
    uistack([errs1,errs2,errs3], 'top');
